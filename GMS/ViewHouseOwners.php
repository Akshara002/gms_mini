...
<style>
	...
	<table>
		<tr>
			<td><h2 class="adjustloc">House Owners</h2></td>
...
<table class="table table-hover table-bordered adjustloctable">
	...
	<tbody>
		...
		{
			$stmt = $con->prepare("SELECT userDetailsId, name, dob, gender, addressline1, addressline22, street, district, city, state, country, pincode, mobile FROM user_details WHERE userTypeId = 2 AND  user_details.isDeleted = 0");
			$stmt->execute();
